document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Limpa opções do select (mantém placeholder)
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // Construir markup incluindo lista de participantes
        const participantsHtml = (details.participants && details.participants.length > 0)
          ...
          : `<p class="participants-none">Nenhum inscrito</p>`;
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        // Preserva a classe "message" e adiciona "success"
